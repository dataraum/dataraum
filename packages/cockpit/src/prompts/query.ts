...
5. ANSWER — once run_steps confirms the query, finish by emitting your answer in the required output structure (the headline number(s) from the validated sample, in plain language). Emit it ONLY when you are done: emitting it is what ends the turn, so do not emit a partial or placeholder answer while you still intend to call another tool. If you genuinely cannot validate a query, emit the answer anyway with a short explanation of why, so the user gets the story rather than nothing.
</reasoning>
