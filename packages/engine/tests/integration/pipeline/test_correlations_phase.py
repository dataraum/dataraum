...
class TestCorrelationsPhase:
    ...
    def test_skip_when_all_tables_analyzed(
        ...
        """``should_skip`` owns the run-scoped redelivery guard (DAT-502).

        The in-``_run`` duplicate of this check is gone — a success-redelivery
        re-executes the whole phase and lands here, where the run-scoped
        DerivedColumn presence skips it.
        """
