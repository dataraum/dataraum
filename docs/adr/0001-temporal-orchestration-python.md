...
- **Amended (DAT-529/530, [ADR-0014](./0014-cockpit-orchestration-worker.md)):** scoped — **analysis** workflows stay Python on the engine worker (this ADR holds for them). The cockpit now ALSO authors + hosts **orchestration** workflows (`JourneyWorkflow`) on its own co-located TS worker (`cockpit-orchestration` queue), which drives the engine analysis workflows as cross-language children. "The cockpit does not author workflows" is now true only of *analysis*.
