...
class TestOverlayAwareLoading:
    ...
    def test_overlay_row_replaces_seeded_spec_by_id(self, session: Session):
        """A teach row's legacy ``parameters.tolerance`` normalizes onto the typed
        field — the LIVE fold (DAT-880 review correction): frame induction still
        writes this shape for the four canonical check types."""
