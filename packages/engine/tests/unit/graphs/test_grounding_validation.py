...
def test_no_connection_degrades_to_lexical_for_every_fragment() -> None:
    """No DuckDB connection (pure-mock callers): the lexical net still runs and
    the phantom check stays off — enforcement degrades, never crashes."""
    out = _output(
        where=["account_type IN ('revenue')"],
        basis=_basis("revenue", ["amount"]),
    )
    violations = validate_grounding_basis(out, _SCHEMA, None)
    assert any("'account_type'" in v and "does not enumerate" in v for v in violations)
    assert not any("never" in v for v in violations)
