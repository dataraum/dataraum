"""Promoted-read surface — head-joined views + grants (docs/architecture/persistence.md, DAT-453).
...
Two read modes (the git frame, docs/architecture/persistence.md): these views are the tracking-branch
...
# failure contract's exempt list (DAT-502 / docs/architecture/persistence.md). The contract: Postgres
    ...
    for table in tables:
        ...
        if has_run_unique and table.name in _RUN_GRAIN_EXEMPT:
            ...
            problems.append(
                ...
                f"storage/read_views.py; failure contract DAT-502/docs/architecture/persistence.md)."
            )
        ...
        if name in _ALWAYS_PASSTHROUGH:
            ...
            if name not in classified:
                raise RuntimeError(
                    ...
                    f"table must appear on the promoted-read surface (docs/architecture/persistence.md)."
                )
...
def dump_read_ddl() -> str:
    ...
    header = (
        ...
        "-- Promoted-read surface (docs/architecture/persistence.md): current_* head-joined views for\n"
...
def ensure_reader_role(connection: Connection, workspace_schema: str, password: str) -> None:
    ...
    The grant is the docs/architecture/persistence.md enforcement: the cockpit's metadata connection
