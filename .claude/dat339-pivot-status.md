...
Active phase branch: `feat/dat-339-pivot-p0-substrate` (0a + 0b)
...
- [ ] **Phase 0c — Starlette + delete FastAPI.** Replace FastAPI app, port `/health`, add 3 `501 Not Implemented` stubs for measure/query/probe. Delete `packages/engine/src/dataraum/api/` entirely. Delete `packages/api/openapi.yaml` + `packages/engine/scripts/export_openapi.py`. NOTE: `Workspace` SQLA model can now also delete (last consumer `api/services.py` is gone in this phase). Drop fastapi from `packages/engine/pyproject.toml`.
  - **Substrate eager-init (carried from 0b).** Current `lifespan` calls `bootstrap_lake` + `bootstrap_workspace` but never instantiates `ConnectionManager`, so the workspace schema + 33 SQLAlchemy tables only materialize lazily when the first request reaches the substrate. The new Starlette lifespan must call `ConnectionManager.initialize()` (or equivalent) at startup so `/health` and downstream routes never race a half-bootstrapped substrate. Acceptance: a fresh `docker compose up --wait` produces `ws_<id>` schema + 33 tables before `/health` is probed; no manual `_init_sqlalchemy` trigger needed.
