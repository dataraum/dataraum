...
def _seed_header_dated_flow(session: Session) -> None:
    """Seed a header-dated flow: ``journal_lines.amount`` anchored on a HEADER date
    served as ``entry_id__date`` — the DAT-812 capability the reconstruction-based first
    cut could not express.

    The anchor axis is NOT a column of the line fact; it lives on the header
    (``journal_entries.date``) and is exposed on the view as ``entry_id__date``. The
    resolver reads it BY IDENTITY: the DAT-778 witness (``MeasureAggregationLineage``)
    pins ``event_time_axis_column_id = journal_entries.date``, the served column with
    that ``source_column_id`` gives the name-in-relation ``entry_id__date`` for the live
    scan, and the cadence comes from that source column's profile. Name reconstruction
    is impossible here — source ``date`` ≠ served ``entry_id__date`` — so a pass proves
    the identity path, not a lucky name match."""
    session.add_all(
        [
            Table(
                table_id="t_jl",
    ...
    session.add(
        ColumnConcept(
            ...
            run_id=RUN,
            temporal_behavior="additive",  # amount resolves to a flow
            annotation_source="llm",
        )
    )
    # The line fact declares NO own anchor — its event date is the header's.
    session.add(
        TableEntity(
            table_id="t_jl",
            run_id=RUN,
    ...
    # The cadence lives on the header date column, keyed by its own column_id.
    session.add(
        TemporalColumnProfile(
    ...
    # The DAT-778 witness: amount reconciles the header's per-period sums; its winning
    # event-time axis id IS journal_entries.date — the anchor read by identity (the
    # slice/measure-axis fields are audit provenance the resolver never reads).
    session.add(
        MeasureAggregationLineage(
            run_id=RUN,
            measure_table_id="t_jl",
            measure_column_id=amount_col.column_id,
            event_table_id="t_je",
            measure_time_axis_column="date",
            measure_time_axis_column_id=None,
            event_time_axis_column="date",
            event_time_axis_column_id=date_col.column_id,
            measure_slice_column_id=amount_col.column_id,
            event_slice_column_id=date_col.column_id,
            slice_dimension="entry",
    ...
    session.add_all(
        [
            ...
            Column(
                table_id="tev_t_jl",
                column_name="entry_id__date",  # the HEADER date served under the FK prefix
                column_position=1,
                origin="dimension",
...
def test_derives_header_dated_flow_window_via_witness_axis(
    integration_engine: Engine,
    pg_session: Session,
    duckdb_conn: duckdb.DuckDBPyConnection,
) -> None:
    """The DAT-812 capability: a flow anchored on a HEADER date (served as
    ``entry_id__date``, NOT a fact column) derives its quarterly window (273 →
    fencepost-corrected 364), NOT the config 30.

    The anchor is resolved BY IDENTITY — the DAT-778 witness id
    (``event_time_axis_column_id`` = ``journal_entries.date``), the served column with
    that ``source_column_id`` gives the name-in-relation ``entry_id__date``, and the
    cadence comes from the header date's profile. ``anchor_time_axis == ['entry_id__date']``
