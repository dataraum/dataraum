#!/bin/sh
# Temporal Postgres schema setup (one-shot, run by the temporal-admin-tools
# service before the server starts). Verbatim from temporalio/samples-server
# compose/scripts/setup-postgres.sh — creates the `temporal` +
# `temporal_visibility` databases in the Postgres pointed at by POSTGRES_SEEDS
# and provisions/upgrades their schemas. We point POSTGRES_SEEDS at the shared
# `postgres` service, so these two databases land alongside dataraum /
# dataraum_lake_catalog / cockpit_db in the one instance.
set -eu
