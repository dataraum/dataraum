...
A run links the typed tables it operated over (:class:`RunTable`); they are read
back through :func:`tables_for_run` rather than re-passed.
"""
...
def link_run_tables(session: Session, run_id: str, table_ids: Iterable[str]) -> int:
    ...
    Writes one ``run_tables`` row per typed table so the terminal sealing steps
    can re-read the run's table set (:func:`tables_for_run`) without re-passing
    it. ``add_source`` and
