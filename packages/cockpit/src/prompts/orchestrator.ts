...
- frame: pick the vertical (domain ontology) the data belongs to — adopt a builtin that already fits, or co-design a new named vertical's MODEL: its business concepts, the validations (data-quality / business-rule checks) over them, AND the metrics (computation DAGs) over them.
...
- Act: frame (co-design the user's model — the business concepts, the validations over them, AND the metrics over them — and declare it under a named vertical), select (register the chosen data as workspace source(s) AND start the import in one approved step), teach (record a correction or declaration), replay (re-run processing for a source).
