...
There is NO in-place text repair (DAT-671): a rewritten statement would silently
diverge from the clause parts / committed grounding it was rendered from. Failures
fail loud; the graph path heals by looping (retained failure → prior_context →
re-author).
...
def execute_sql_steps(
    ...
    *,
    ...
    """Execute SQL steps as CTEs and run the final SQL.

    A failing statement FAILS — there is no in-place text repair (DAT-671):
    a rewritten statement would silently diverge from the clause parts and the
    committed grounding it was rendered from. The graph path heals by LOOPING
    (retained failure → prior_context → re-author), never by mutation.
    ...
    # `WITH <prior…>, <this step> SELECT * FROM <this step>` — not in raw isolation.
    ...
    # Compose the single standalone statement (steps as CTEs + final_sql) —
    # the deterministic executable, no temp views.
...
def _execute_step(
    ...
    matching the prior `SELECT * FROM <view>` semantics. A failure is a failure —
    no text repair (DAT-671).
    """
    try:
        ...
        return Result.ok(
            StepExecutionResult(
                ...
                sql_executed=step.sql,
                value=value,
            )
        )
    except Exception as e:
        return Result.fail(f"Step '{step.step_id}' failed: {e}")
...
def _execute_final(
    ...
    try:
        if return_table and display_limit is not None:
            ...
            limited_sql = f"SELECT * FROM ({final_sql}) AS _dr_limited LIMIT {display_limit}"
            ...
            count_row = duckdb_conn.execute(
                f"SELECT COUNT(*) FROM ({final_sql}) AS _dr_count"
            ).fetchone()
