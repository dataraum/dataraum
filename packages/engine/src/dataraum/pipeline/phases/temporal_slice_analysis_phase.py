...
Per-(slice table, period) row counts + numeric-column sums — the
aggregation-lineage reconciliation substrate (DAT-491). One ``GROUP BY`` over
each slice table's time column; periods come from the data.
"""
...
class TemporalSliceAnalysisPhase(BasePhase):
    """Per-(slice, period) sums on slices (DAT-491 lineage substrate).

    One ``GROUP BY`` per slice table over its time column → per-period row count
    and numeric-column sums, persisted as ``TemporalSliceAnalysis`` rows.
    ...
    def _run(self, ctx: PhaseContext) -> PhaseResult:
        """Compute per-(slice, period) sums on slices."""
        # Source-free: the session's selected typed tables (DAT-403).
        typed_tables = self._typed_tables(ctx)
        ...
        if not slice_definitions:
            return PhaseResult.success(
                outputs={"message": "No slice definitions found", "period_analyses": 0},
                records_processed=0,
                records_created=0,
            )
        ...
        if not table_time_columns:
            return PhaseResult.success(
                outputs={"message": "No temporal column found for any table", "period_analyses": 0},
                records_processed=0,
                records_created=0,
            )
        ...
        # Each physical slice table is summed exactly once per run. KEPT even
        # with the ``uq_slice_def_table_column_run`` grain (DAT-502): sanitized
        # slice prefixes are not prefix-free — ``account``'s prefix also matches
        # ``account_type``'s slice tables — so two DISTINCT definitions can route
        # the same physical slice table here twice. Persisting its periods twice
        # in one activity would violate ``uq_tsa_slice_period_run``. The sums are
        # independent of which definition routed us here (same source ⇒ same time
        # column), so deduping is loss-free.
        ...
        for slice_def in slice_definitions:
            ...
            time_column, _time_profile = tc_entry
            ...
            slice_names = [
                st.table_name for st in all_slice_tables if st.table_name.lower().startswith(prefix)
            ]
            if not slice_names:
                continue

            for slice_table_name in slice_names:
                if slice_table_name in processed_slice_tables:
                    continue
                processed_slice_tables.add(slice_table_name)
                try:
                    sums = compute_period_sums(
                        slice_table_name=slice_table_name,
                        time_column=time_column,
                        grain=grain,
                        duckdb_conn=ctx.duckdb_conn,
                    )
                    if sums.success and sums.value is not None:
                        persisted = persist_period_sums(
                            periods=sums.value,
                            slice_table_name=slice_table_name,
                            time_column=time_column,
                            session=ctx.session,
                    ...
                    elif not sums.success:
                        errors.append(f"{slice_table_name}: {sums.error}")
                except Exception as e:
                    errors.append(f"Analysis error for {slice_table_name}: {e}")
        ...
        return PhaseResult.success(
            ...
            records_created=total_period_analyses,
            summary=f"{total_period_analyses} period analyses",
        )
