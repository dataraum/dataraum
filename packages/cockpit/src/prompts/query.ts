...
snippet_search returns each snippet's concept keys (standard_field / statement / aggregation), its validated SQL (the canonical computation), and column_mappings_basis (how each concept was grounded: measure_columns, filter_columns, filter values). Match a step to a snippet by the concept keys, then:
...
Fill the source in when the step is ONE aggregate over ONE table with optional filters. Split it exactly as it appears in your SQL: relation is the lake.<layer>.<name> you read, value_expr is the aggregate WITHOUT the AS value alias, filters is one string per predicate you would join with AND.
...
- A table marked (periodic snapshot) in <entities> records period-end LEVELS (a stock, e.g. a trial balance), not per-period flows — NEVER SUM its measures across periods (that double-counts). Aggregate WITHIN one period, or take the latest period's value; SUM across time only for a plain (fact) table's additive measures.
