...
> **Worker code is NOT hot-reloaded.** The orchestration worker is a `globalThis`-pinned singleton bundled once at `Worker.create` (`src/worker/worker.ts`); HMR re-imports the module but reuses the running worker. So edits to **`src/worker/`** (the orchestration workflows, contracts, activities) do **not** take effect under `bun --bun run dev` — **restart the dev server** to load them. (The prod/container face is the deploy step under "Temporal — two workers" below.)
...
TanStack **Start** (React 19) · **Router** / **Query** · **Mantine v9** + **Tailwind v4** · **Drizzle** + `postgres` · **Temporal** (`@temporalio/client` triggers the Python engine workflows; `@temporalio/worker` runs the co-located TS **orchestration** worker — DAT-529) · **Biome** · **Vitest** · strict TypeScript. The wider ecosystem (TanStack AI, xyflow, Vega-Lite — see Charting below — CodeMirror, Arrow JS) lands as widgets need it.

## Charting

**Vega-Lite is the only charting library** — no ECharts, no wrapper deps. Agent-authored
chart configs are a thin LLM-authorable Vega-Lite subset (`src/charts/chart-config.ts`);
the gate a config passes before rendering is zod-subset + column check + vega-lite
`compile()` (`src/charts/validate.ts`) — never a bundled JSON Schema. Rendering is a thin
DOM shell over vega + vega-lite (`chart-view.tsx`); charts re-run live SQL, they never
embed data.
...
- **Temporal (DAT-344):** the **analysis** workflows + activities are **Python, bundled on the engine worker** — a server function calls `client.workflow.start("addSourceWorkflow", …)` via `@temporalio/client` and renders progress. **DAT-529 amends this, scoped:** the cockpit now ALSO authors + runs **orchestration / control-plane** workflows on a **co-located TS worker** — a module-level singleton started at server boot (Nitro plugin, `src/server/plugins/orchestration-worker.ts`), polling the `cockpit-orchestration` queue. The workflows are **short-lived + per-trigger** (DAT-609): `groundingLoopWorkflow` (import + teach loop) and `sessionCascadeWorkflow` (begin_session → operating_model); `replay` + `operating_model` are direct single-shot engine starts. Analysis stays Python; only orchestration is TS. See the `feedback-durable-execution-lean` memory.
...
- **Temporal — two workers, scoped (DAT-529 amends DAT-344; DAT-609):** **analysis** workflows are **Python** on the engine worker; the cockpit `@temporalio/client` starts/queries them (`packages/engine/src/dataraum/worker/workflows.py`). **Orchestration** workflows are **TypeScript** on the cockpit's own co-located worker (`@temporalio/worker` + `@temporalio/workflow`, `src/worker/`): **short-lived per-trigger** workflows — `groundingLoopWorkflow` (`grounding-<ws>`) and `sessionCascadeWorkflow` (`session-<ws>`) — started by a deterministic per-workspace id, polling `cockpit-orchestration`. (`replay` + `operating_model` skip orchestration — direct single-shot engine starts via `temporal/orchestration-trigger.ts#startDirectRun`.) Single-flight = the workflow-id reuse policy (`ALLOW_DUPLICATE` + conflict `FAIL`), NOT a resident coordinator. The worker pulls a native Rust **core-bridge** — so the runner is **no longer pure-alpine**: `@temporalio/*` is externalized from the bundle and the workflow code is pre-bundled at build time (`scripts/build-workflow-bundle.ts` → `workflowBundle`, the SDK's prod path — never `workflowsPath` in prod), with the worker's runtime deps shipped via a prod-deps Docker stage. The worker runs under **Bun ≥ 1.3.14** (Bun-worker support landed in SDK 1.15; one benign caveat — `promiseHooks` is unavailable, so the built-in `__stack_trace` query is disabled). Sandbox rule: `src/worker/workflows/` is deterministic, imports only `@temporalio/workflow` + pure modules (`contracts.ts`, `run-stage.ts`, `grounding-step.ts`, the `workflow-id` helpers); all IO lives in `src/worker/activities.ts`. **Deploying a worker-code change:** the bundle is baked into the image, so recreate the container — `docker compose -f packages/infra/docker-compose.yml up -d --build --force-recreate cockpit`. The workflows are short-lived, so a new trigger simply runs the new code — **no `patched()` discipline, no continue-as-new, no deploy-on-next-signal** (all retired with the singleton, DAT-609). The real trap is the **cached-image / no-recreate** path: `up` without rebuild+`--force-recreate` leaves the old worker on the old bundle and the change silently never lands. Full model → `docs/architecture/orchestration.md`.
