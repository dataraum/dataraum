...
//
// runId provisional-vs-final: Temporal mints the execution `runId` only at
// `workflow.start`, so the pre-start call records the run keyed by its
// deterministic `workflowId` with `runId` left as the workflowId placeholder;
// `attachRunId` rewrites it to the real execution id right after start (a no-op
// best-effort touch — the orphan-critical session + run rows already exist).
