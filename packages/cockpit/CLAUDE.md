...
- **React 19 has NO Intent skill** — the authority is <https://react.dev> (fetch it; never write React idioms from training-data memory). Project-distilled rules: the "UI quality bar" and the "React idiom" section below (both binding).
...
- **Config data** (vertical YAMLs) is bind-mounted read-only at `DATARAUM_CONFIG_PATH`; read via Node `fs` through `config.dataraumConfigPath` — never imported, never path-navigated from elsewhere. Consumers: `tools/list-verticals.ts`, `tools/frame-family.ts`, the `teach-{cycle,metric,validation}` overlay writers, and `prompts/conventions.ts`.

## Skills & conventions

- **External skills:** Temporal — `temporal-developer` is already installed at `.claude/skills/temporal-developer/` and auto-activates; nothing to install. (TanStack guidance is the official **Intent** skills — see the Skill Loading block at the top of this file, not a vendored skill.)
