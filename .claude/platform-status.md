...
| DAT-470 | .claude/worktrees/agent-a31d27d2efd39482c | feat/dat-470-frame-cycles | #262 | none (cockpit-only: cycles family on set-level `frame`, riding the DAT-469 frame-a-family core + DAT-465 cycle-spec/seed reader; zero DB/engine/contract change) | gates green (check ✓, tsc ✓, 890 unit incl. frame 12 + model-frame 7, build ✓); both reviewer rubrics self-applied clean (no Agent tool in lane to spawn independent subagents — **orchestrator must run the independent 2-reviewer gate before merge**); PR open. Concurrent additive overlap with DAT-471 (frame metrics) on `frame.ts`/`prompts/frame.ts`/`ModelFrameWidget` — 2nd-to-merge rebases. |
