...
|---|---|---|---|---|---|
| DAT-471 | .claude/worktrees/agent-a62d92129f276f4f2 | feat/dat-471-frame-metrics | (pending) | none (cockpit-only; consumes DAT-466 MetricSpecSchema/teach_metric + DAT-469 frame-a-family core, both merged on main) | gates green (tsc ✓, biome ✓, 894 vitest ✓, build ✓), inline 2-rubric review PASS. AC3 goal-direction FINDING: engine `processor.py` reads file-only `load_all()`, NOT overlay — framed-metric goal-direction needs a 1-line engine swap (out of cockpit scope; documented in PR). Conflicts-at-merge with DAT-470 (frame cycles) on frame.ts/model-frame.tsx/prompts — additive, 2nd-to-merge rebases. |
