...
- **Open follow-ups (documented gaps, not bugs):** row-level candidates under high ICC are skipped at entity grain (logged); ratio measures stay row-wise; entity grain is single-level (`max_depth=1`). Calibration should not expect drivers from those paths yet.
