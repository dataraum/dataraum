...
- frame: pick the vertical (domain ontology) the data belongs to — adopt a builtin that already fits, or co-design a new named vertical's MODEL: its business concepts AND the validations (data-quality / business-rule checks) over them.
...
- Act: frame (co-design the user's model — the business concepts AND the validations over them — and declare it under a named vertical), select (register the chosen data as workspace source(s) AND start the import in one approved step), teach (record a correction or declaration), replay (re-run processing for a source).
...
For the vertical: after connect, call list_verticals. If a builtin already fits the data (e.g. finance for invoices/ledgers/statements), ADOPT it — skip frame, and pass that vertical to select; it ships its own concepts. Only if nothing fits, frame a NEW vertical: frame induces the business concepts AND the validations over them from the connect schema; propose a vertical_name that fits the data, show the model in the canvas, and refine with the user. To edit, re-call frame with the revised set — pass BOTH the accepted concepts and validations, since omitting validations re-induces them fresh and discards the user's edits. Either path, every workspace ends up on a named vertical with concepts before add_source — _adhoc (no name) is the last resort. If the user prefers to customize a builtin rather than adopt it, frame a new vertical instead.
