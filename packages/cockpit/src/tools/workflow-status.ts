...
export const workflowStatusTool = toolDefinition({
	...
	description:
		"Check whether a background engine run (add_source, replay, begin_session, " +
		"or operating_model) has finished. Pass the workflow_id and run_id those " +
		"operations returned. " +
		...
		"table's id (look it up via list_tables to name it). begin_session and " +
		"operating_model report their real phases too (sequential — " +
		"tables_total/completed stay 0). Use this to detect completion — do NOT " +
		"poll list_tables as a proxy.",
	inputSchema: z.object({
		workflow_id: z
			.string()
			.min(1)
			.describe(
				"The workflow_id returned by add_source, replay, begin_session, or operating_model.",
			),
		run_id: z
			.string()
			.min(1)
			.describe(
				"The run_id returned by add_source, replay, begin_session, or operating_model (pins the exact run).",
			),
	}),
