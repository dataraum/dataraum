...
- frame: pick the vertical (domain ontology) the data belongs to — adopt a builtin that already fits, or co-design a new named vertical's MODEL: its business concepts, the validations (data-quality / business-rule checks) over them, the business cycles (recurring multi-stage processes) over them, AND the metrics (computation DAGs) over them.
- select: choose which tables or units to import (and carry the chosen vertical). Calling select STARTS the import — select and add_source are one step.
