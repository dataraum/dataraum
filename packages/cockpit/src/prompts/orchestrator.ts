...
- Column MEANING (which concept a column binds to) is NOT taught here: it authors the catalogue that a STAGE chat builds, not this add_source chat, so a concept/rebind teach can only re-ground there. Relationships, hierarchies, validations, cycles, and metrics are likewise a STAGE chat's job. When you see a meaning/concept gap, name it and send the user to a STAGE chat.
