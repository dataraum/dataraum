...
|---|---|---|---|---|---|
| DAT-471 | .claude/worktrees/agent-a62d92129f276f4f2 | feat/dat-471-frame-metrics | #263 | none (cockpit-only; consumes DAT-466 MetricSpecSchema/teach_metric + DAT-469 frame-a-family core, both merged on main) | gates green (tsc ✓, biome ✓, vitest ✓, build ✓), inline 2-rubric review PASS. AC3 goal-direction FINDING: engine `processor.py` reads file-only `load_all()`, NOT overlay — framed-metric goal-direction needs a 1-line engine swap (out of cockpit scope; documented in PR). Merged origin/main (DAT-470 #262 frame cycles) — additive four-family resolution: frame now induces concepts + validations + cycles + metrics. PR open — awaiting lead merge. |
