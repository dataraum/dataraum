...
class AddSourceWorkflow:
    ...
    def get_progress(self) -> ProgressSnapshot:
        ...
        cockpit Client polls this by workflow id (queried per ``run_id``; a
        ReplayScope replay reuses the id under ALLOW_DUPLICATE and resets
        progress per run). The snapshot carries the per-table ``tables`` steps
        and any ``failure`` directly, so the cockpit needs no per-child queries.
        """
    ...
    async def _run_inner(self, payload: AddSourceInput) -> AddSourceResult:
        ...
        # Seed the per-table steps as "running" before any child is awaited
        # (import recorded ``target_raw_ids`` in history → deterministic), so an
        # early query already sees the named steps behind the count, not just a
        # total. Each flips to "done"/"failed" as its child resolves.
...
async def _maybe_session_replay_cleanup(
    ...
    re-run so its ``should_skip`` doesn't bail. Mirrors the add_source
    ``_maybe_replay_cleanup`` guard shape: NOTHING is cleaned on the initial run
    (``replay is None``) — only on a replay, and only for phases at-or-after
    ``from_phase``. (Do NOT use ``_runs_under`` here: it returns True for the
    initial run, which would wrongly fire cleanup before the first real run.)
    """
