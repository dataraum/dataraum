...
class ConformVerdict(BaseModel):
    ...
    concept_label: str = Field(
        description=(
            "Canonical concept label (lowercase, singular) — mandatory on a conform "
            'verdict, "" on every other verdict (DAT-807: the not-applicable case is '
            "a documented empty value, not an omitted key)"
        ),
    )
    ...
    def _conform_requires_label(self) -> ConformVerdict:
        ...
        Enforced at the model so the batch fails LOUD — the consumer must never
        complete an LLM judgment field deterministically (a column name is not a
        concept label). Constrained decoding guarantees the SHAPE, not this
        cross-field contract, so this is the one validator that can still reject
        a structurally-valid payload; the lane is then skipped and the stats'
        verdicts stand (the module's posture rule).
        """
...
class ConformBatchOutput(BaseModel):
    """Structured output: one verdict per submitted candidate pair.

    Emptiness is NOT constrained here — it is checked by the caller. A schema
    ``min_length`` would make an honest "no judgment" impossible to express:
    the only way to satisfy the constraint is to INVENT verdicts, and a
    fabricated verdict on a guessable ref is far worse than the silence it
    would replace (DAT-725 review).
    """
...
class AliasIdentityBatchOutput(BaseModel):
    """Structured output: one identity verdict per submitted bijection pair.

    Emptiness is checked by the caller, never here — see
    ``ConformBatchOutput``. The stakes are higher on this batch: alias refs are
    ``str(i)`` ("0", "1", …), so an invented verdict would land on a REAL pair,
    and one above the merge floor collapses two drill axes into one dimension —
    the exact corruption this module exists to prevent (DAT-725 review).
    """
...
class DimensionIdentityJudge(LLMFeature):
    """Structured-output judge over the DAT-762 cross-fact conform judgment."""
    ...
    def _judge[T: BaseModel](
        ...
        *,
        ...
        """One judgment turn under constrained decoding (DAT-807).

        The verdicts arrive as structured-output content the API constrained to
        ``output_model``'s schema, so shape is guaranteed. What is NOT guaranteed
        is the model's cross-field contract (``ConformVerdict`` requires a
        ``concept_label`` on a conform verdict) — that stays a loud failure.
        """
